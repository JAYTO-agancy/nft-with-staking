...
const addRarity = () => {
  ...
  const rareColor = [
    {
      name: "Hot Dog",
      rgb: { r: 192, g: 158, b: 131 },
    },
    {
      name: "Hot Dog",
      rgb: { r: 128, g: 134, b: 90 },
    },
    {
      name: "Hot Dog",
      rgb: { r: 113, g: 65, b: 179 },
    },
    {
      name: "Hot Dog",
      rgb: { r: 162, g: 108, b: 67 },
    },
  ];
...
const saveMetadata = (
  ...
  let shortName =
    _loadedImageObject.imgObject.filename.replace(
      /\.[^/.]+$/,
      ""
    );
...
const startCreating = async () => {
  ...
  if (images == null) {
    console.log(
      "Please generate collection first."
    );
    return;
  }
  let loadedImageObjects = [];
  images.forEach((imgObject) => {
    loadedImageObjects.push(
      loadImgData(imgObject)
    );
  });
  await Promise.all(
    loadedImageObjects
  ).then((loadedImageObjectArray) => {
    loadedImageObjectArray.forEach(
      (loadedImageObject) => {
